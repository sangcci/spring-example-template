...
위 호출은 재가입 허용 여부라는 하나의 업무 결정을 표현한다. 최근 탈퇴 여부와 테스트 계정 예외처럼 그 결정을 구성하는 조건은 독립적인 책임과 이름이 있을 때 Policy 안에 둔다. boolean 결과를 받은 호출자가 언제나 같은 업무 예외를 던진다면, `isBlocked`보다 `ensureRejoinAllowed` 또는 `rejectIfRejoinBlocked`처럼 허용 또는 거절의 결과를 직접 표현하는 이름을 사용한다.
